...
function didPassStart(from,steps){
...
function simulate(start,order){
  let pos=start,coins=0;
  const pathDetails=[];
  const firedMoveCells=new Set(); // 이번 simulate에서만 유효 (다음 바퀴=다음 calculate 호출 시 초기화)

  for(const d of order){
    const landed=stepForward(pos,d);

    let passBonus=0;
    if(didPassStart(pos,d)) passBonus=coinVals[1];
    ...
    if(sp){
      if(sp.type==='mystery'){
        ...
        if(firedMoveCells.has(landed)){
          // 이미 이번 바퀴에 발동된 이동 발판 → 성장비료 100
          stepCoins=100;finalPos=landed;moveBlocked=true;
        }else{
          ...
          while(true){
            ...
            if(cur!==landed&&firedMoveCells.has(cur)){stepCoins=100;finalPos=cur;moveBlocked=true;break;}
      ...
      stepCoins=coinVals[landed]||0;
    }

    coins+=stepCoins+passBonus;
    pathDetails.push({from:pos,dice:d,landed,teleFrom,dest:finalPos,coins:stepCoins+passBonus,passBonus,moveBlocked});
    pos=finalPos;
  }
...
function calculate(){
  ...
  const perms=permutations([...activeDice]);
  const seen=new Set();
  const results=[];
  for(const perm of perms){
    ...
    seen.add(key);
    results.push({order:perm,...simulate(currentPos,perm)});
  }
...
function displayResults(results){
  ...
  document.getElementById('bestOrder').innerHTML=best.order.map((d,j)=>
    `<span style="display:inline-flex;align-items:center;vertical-align:middle;color:var(--gold)">${diceSvg(d,16)}</span>${j<best.order.length-1?'<span style="font-size:10px;color:var(--text3);margin:0 1px">→</span>':''}`
  ).join('');
  ...
  results.forEach((r,i)=>{
    ...
    const pathStr=r.pathDetails.map(d=>{
      let s='';
      if(d.moveBlocked) s=`${d.dest}번(🔒+100)`;
      else if(d.teleFrom!==null){const sp=getSpecial(d.teleFrom);const sign=sp.moveVal>=0?'+':'';s=`${d.teleFrom}번(${sign}${sp.moveVal}칸)→${d.dest}번(+${coinVals[d.dest]||0})`;}
      else{const sp=getSpecial(d.dest);s=sp&&sp.type==='mystery'?`${d.dest}번(?)`:`${d.dest}번(+${coinVals[d.dest]||0})`;}
    ...
    item.innerHTML=`
      ...
      <div class="rbody">
        <div class="rorder">
          ${r.order.map((d,j)=>`<span style="display:inline-flex;align-items:center;vertical-align:middle;color:var(--gold)">${diceSvg(d,16)}</span>${j<r.order.length-1?'<span class="rarr">▶</span>':''}`).join('')}
